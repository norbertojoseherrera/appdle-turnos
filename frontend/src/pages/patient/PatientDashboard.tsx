...
export default function PatientDashboard() {
  ...
  const AppointmentCard = ({ a, showCancel }: { a: Appointment; showCancel?: boolean }) => (
    <div className="bg-white rounded-xl border border-gray-200 p-4">
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex flex-col items-end gap-2 shrink-0">
          ...
          {showCancel && a.status !== 'cancelled' && a.status !== 'completed' && (
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl md:text-2xl font-bold text-gray-900">Mis turnos</h1>
      ...
      <section className="mb-8">
        <h2 className="text-base md:text-lg font-semibold text-gray-700 mb-3">Próximos turnos</h2>
        {upcoming.length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-100 p-6 text-center">
            <p className="text-gray-400 text-sm mb-3">No tenés turnos próximos.</p>
            <Link to="/appointments/new">
              <Button size="sm">Reservar tu primer turno</Button>
            </Link>
          </div>
        ) : (
          <div className="space-y-3">
            {upcoming.map((a) => <AppointmentCard key={a.id} a={a} showCancel />)}
          </div>
        )}
      </section>

      <section>
        <h2 className="text-base md:text-lg font-semibold text-gray-700 mb-3">Historial</h2>
        {past.length === 0 ? (
